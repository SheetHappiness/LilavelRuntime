...
| --- | --- | --- |
| Persistent agent kernel | Python 3.12+ `apps/runtime` | Top-level process lifecycle, bounded event ingress, environment task ownership, structural tool registration, and inert wake classification. |
...
The current stack contains no scheduler, world model, attention engine, tool
executor, retrieval/memory layer, or durable cross-environment agent-state
store. Those are future design surfaces, not hidden dependencies.
